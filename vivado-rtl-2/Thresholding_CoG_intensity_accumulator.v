...
always @(posedge clk) begin
  ...
  // Assertions: keep the fixed-point accumulator in the expected range.
  always @(posedge clk) begin
    if (!reset && valid) begin
      if (data_in > 8'hFF) $error("pixel input overflow");
      if (count_pixel_h > NUM_PIXELS_SUBAPETURE_SQRT-1) $error("local_x out of range");
      if (count_pixel_v > NUM_PIXELS_SUBAPETURE_SQRT-1) $error("local_y out of range");
      if (i_next > 16'd65280) $error("si_sum overflow: %0d", i_next);
      if (x_i_next > 19'd489600) $error("mx_sum overflow: %0d", x_i_next);
      if (y_i_next > 19'd489600) $error("my_sum overflow: %0d", y_i_next);
    end
  end
endmodule
