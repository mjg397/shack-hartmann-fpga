...
//   Slopes     : 25-bit signed fixed-point input
//   Products   : 43-bit signed  (18 + 25 bits, exact)
//   x+y sum    : 44-bit signed  (one guard bit)
//   Accumulator: sign-extended to 52 bits
//   Output     : 25-bit signed fixed-point output (bits [42:18] of accumulator)
//
...
module ematrix_accumulator #(
    ...
    // flattened output bus: mode m occupies bits [m*25 +: 25]
...
initial begin
    $readmemh("data/e_matrix_x.hex", e_rom_x);
    $readmemh("data/e_matrix_y.hex", e_rom_y);
end
...
generate
    for (m = 0; m < NUM_MODES; m = m + 1) begin : mac_gen
        ...
        // 18-bit signed × 25-bit signed = 43-bit signed (exact, no overflow)
        wire signed [42:0] prod_x = ex * $signed(x_slope);
        wire signed [42:0] prod_y = ey * $signed(y_slope);

        // Sign-extend each 43-bit product to ACC_WIDTH using the product MSB [42]
        assign mac_sum[m] = {{9{prod_x[42]}}, prod_x}
                          + {{9{prod_y[42]}}, prod_y};
    end
endgenerate
...
always @(posedge clk) begin
    if (rst) begin
        ...
        case (state)
            ...
            STATE_IDLE: begin
                if (sub_valid) begin
                    ...
                    if (sub_counter == LAST_SUB) begin
                        ...
                        // truncate accumulator → 25-bit signed fixed-point by selecting bits [42:18]
                        for (i = 0; i < NUM_MODES; i = i + 1) begin
                            acc[i]                  <= acc_next[i];
                            zernike_out[i*25 +: 25] <= acc_next[i][42:18];
                        end
                        done  <= 1;
                        state <= STATE_DONE;
                    end
            ...
            // ----------------------------------------------------------------
            // STATE_DONE — reset for the next accumulation frame
            // ----------------------------------------------------------------
            STATE_DONE: begin
                sub_counter <= 0;
                for (i = 0; i < NUM_MODES; i = i + 1)
