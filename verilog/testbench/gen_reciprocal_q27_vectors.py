#!/usr/bin/env python3
"""
Reference model for reciprocal_u16_q27: simulates the exact fixed-point
hardware arithmetic and generates golden test vectors.

Output: vectors/reciprocal_golden_q27.memh
  One 48-bit word per input v in [0, 65535], hex-formatted as 12 digits:
    bits[47:32] = v_u16         (input)
    bits[31:27] = 5'b0          (unused padding)
    bits[26: 0] = expected reciprocal_q27 (output)

Usage: python3 gen_reciprocal_q27_vectors.py
"""
...
MASK27 = (1 << 27) - 1   # 0x7FFFFFF
MASK28 = (1 << 28) - 1   # 0xFFFFFFF
...
def newton_step_q27(a_q1_26, x0_u27):
    """
    Simulate newton_step_q27 exactly.
      a_q1_26 : 27-bit Q1.26  (normalised input in [1, 2))
      x0_u27  : 27-bit        (Q0.27 seed / previous estimate)
    Returns (x1_q0_27, saturated) as Python ints.

    Mirrors the Verilog:
      ax_mul      = a_q1_26 * x0_u27            -- 54-bit
      ax_q1_26    = {1'b0, ax_mul[53:27]}        -- >> 27, 28-bit
      clamp to 2.0 in Q1.26 = 0x8000000
      two_minus_ax = 0x8000000 - ax_clamped
      prod_mul    = x0_u27 * two_minus_ax        -- 55-bit
      x1_ext      = (prod_mul + 2^25) >> 26      -- round-to-nearest
      saturate to MASK27
    """
    # ax = (a * x0) >> 27  ->  Q1.26 (27-bit result, zero-extended to 28-bit)
    ax_mul   = a_q1_26 * x0_u27          # up to 54 bits
    ax_q1_26 = ax_mul >> 27              # keep lower 27 significant bits

    # Clamp ax to 2.0 (0x8000000 in Q1.26) before subtraction
    TWO_Q1_26  = 0x8000000
    ax_clamped = min(ax_q1_26, TWO_Q1_26)

    # two_minus_ax in Q1.26 (28-bit)
    two_minus_ax = TWO_Q1_26 - ax_clamped

    # prod = x0 * (2 - ax)  ->  55-bit
    prod_mul = x0_u27 * two_minus_ax

    # Round-to-nearest: add 2^25, then >> 26
    bias   = 1 << 25
    x1_ext = (prod_mul + bias) >> 26

    saturated = x1_ext > MASK27
    x1_q0_27  = MASK27 if saturated else x1_ext
    return x1_q0_27, saturated


def reciprocal_u16_q27(v):
    """
    Full reference model for reciprocal_u16_q27.
    Returns (result_q27, divide_by_zero, saturated).

    Mirrors reciprocal_u16_q27.v exactly.
    """
    v &= 0xFFFF

    if v == 0:
        return MASK27, True, True

    # Normalize to Q1.15 in [1, 2)
    shift_left = clz16(v)
    a_q1_15    = (v << shift_left) & 0xFFFF

    # Zero-extend to Q1.26 (27-bit: 1 integer + 26 fractional)
    a_q1_26 = (a_q1_15 << 11) & MASK27

    # LUT lookup: lut_idx = a_q1_15[14:7]
    lut_idx    = (a_q1_15 >> 7) & 0xFF
    seed_q0_16 = SEED_LUT_Q0_16[lut_idx]

    # Zero-extend seed to Q0.27
    x0_u27 = (seed_q0_16 << 11) & MASK27

    # Two Newton refinement steps
    x1_q0_27, _ = newton_step_q27(a_q1_26, x0_u27)
    x2_q0_27, _ = newton_step_q27(a_q1_26, x1_q0_27)
    ...
    if msb_index == 0:
        ...
        # denorm_round_bias = 27'h1 << (msb_index - 1)  (27-bit, max shift 14)
        bias  = (1 << (msb_index - 1)) & MASK27
        numer = (x2_q0_27 + bias) & MASK28   # 28-bit addition
        out_ext = numer >> msb_index

    out_sat = out_ext > MASK27
    out_q27 = MASK27 if out_sat else out_ext
    return out_q27, False, out_sat


def main():
    out_dir = os.path.join(os.path.dirname(__file__), "vectors")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "reciprocal_golden_q27.memh")

    # Directed test cases matching the reciprocal testbench
    directed = [0, 1, 2, 3, 7, 255, 256, 257, 1023, 43692, 65535]
    print("Directed test case expected values:")
    print(f"  {'v':>6}  {'q27':>10}  {'q27_hex':>10}  {'div0':>4}  {'sat':>4}")
    for v in directed:
        q27, div0, sat = reciprocal_u16_q27(v)
        print(f"  {v:>6}  {q27:>10}  0x{q27:07X}  {int(div0):>4}  {int(sat):>4}")

    print(f"\nGenerating {out_path} ...")
    with open(out_path, "w") as f:
        for v in range(65536):
            q27, _, _ = reciprocal_u16_q27(v)
            # Pack as 48-bit word: upper 16 bits = v, bits [26:0] = q27
            word = ((v & 0xFFFF) << 32) | (q27 & MASK27)
            f.write(f"{word:012X}\n")
