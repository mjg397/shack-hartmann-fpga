...
EW        = E @ W
I_error   = np.linalg.norm(EW - np.eye(NUM_ZERNIKE))
quant_rms = np.sqrt(np.mean((E - E_fp.astype(float) / scale)**2))

print(f"E matrix  shape : {E.shape}  (N x 2M)")
# print(f"          range : [{E.min():.6f}, {E.max():.6f}]")
# print(f"  ||E*W - I||_F : {I_error:.2e}  (0 = perfect reconstruction)")
# print(f"  quant RMS err : {quant_rms:.2e}  (Q1.{FRAC_BITS}, {WORD_BITS}-bit)")
# print(f"  BRAM words    : {total_words}  ({total_words * WORD_BITS / 8192:.2f} kB)")
# print(f"  Tikhonov α    : {alpha:.2e}  (rcond={TIKHONOV_RCOND}, σ_max={sigma_max:.4f})")
